...
export default function BackgroundPage() {
  ...
  if (!countryId) {
    router.push('/first-steps/one'); // Evita que o jogador entre sem escolher um país
    return null;
  }
    ...
    <div className="flex flex-col items-center space-y-4 p-6 w-full h-full">
      ...
      {backgrounds.map((b) => (
        <div className='flex flex-col border border-rose-700 p-5 w-1/3 h-4/5 gap-2 justify-between'>
            ...
            <button
            key={b.id}
            className={`w-full px-4 py-2 border border-rose-700 ${choosedBackground === b.id ? 'bg-rose-700 text-white' : ''} hover:cursor-pointer`}
            onClick={() => {
                chooseBackground(b.id);
            }}
            >
            Select
            </button>
